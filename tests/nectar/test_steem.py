...
class Testcases(unittest.TestCase):
    ...
    def test_transfer(self):
        ...
        tx = acc.transfer("test", 1.33, acc.blockchain.sbd_symbol, memo="Foobar", account="test1")
    ...
    def test_sp_to_rshares(self):
        stm = self.bts
        rshares = stm.sp_to_rshares(stm.vests_to_sp(1e6), post_rshares=1e19)
    ...
    def test_rshares_to_vote_pct(self):
        ...
        for vote_pct in range(500, 10000, 500):
            ...
            vote_pct_ret = stm.rshares_to_vote_pct(
                rshares, steem_power=sp, voting_power=voting_power
            )
    ...
    def test_switch_blockchain(self):
        bts = Steem(node=get_steem_nodes(), num_retries=10)
